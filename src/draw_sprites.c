...
/*   Updated: 2020/07/08 12:32:53 by nkuipers      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void			make_sprites(t_info *info, t_data *data)
{
	...
	{
		info->spi.spx = sprites[i].x - info->rays.posy;
		info->spi.spy = sprites[i].y - info->rays.posx;
		...
		info->spi.sprw = abs((int)(info->det.resy / (info->spi.tfy)));
		info->spi.drsx = -info->spi.sprw / + info->spi.ssx;
		if (info->spi.drsx < 0)
			info->spi.drsx = 0;
